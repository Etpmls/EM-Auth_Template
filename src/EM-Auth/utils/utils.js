    ...
    if (route.component) {
      if (route.component === 'Layout') {
        ...
        route.component = (resolve) =>
          require(['@/EM-Auth/layout/EmptyLayout'], resolve)
      } else {
...
export function deleteConfirmMessage(_this, message, doThen, type = 'warning', title = _this.$t('em-auth.warning'), confirmButtonText = _this.$t('em-auth.submit'), cancelButtonText = _this.$t('em-auth.cancel')) {
...
export function getlang(_this, field) {
  return _this.$t('em-auth.' + field)
}
export function getlang_Route(_this, field) {
  return _this.$t('route_em-auth.' + field)
}
