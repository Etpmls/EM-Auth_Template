...
<script>
import { CategoryGetAll, CategoryDelete } from '@/EM-CMS/api/api'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      if (row.id) {
        deleteConfirmMessage(this, this.lang('etp_message.category_delete'), async() => {
          const { message } = await CategoryDelete({
        ...
        if (this.selectRows.length > 0) {
          ...
          deleteConfirmMessage(this, this.lang('etp_message.category_delete'), async() => {
            const { message } = await CategoryDelete({ categories: ids })
            successMessage(this, this.lang('success'), message)
            this.fetchData()
          })
        } else {
